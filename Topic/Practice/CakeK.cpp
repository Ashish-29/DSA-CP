#include <iostream>

using namespace std;

int main()

{
    char ch = '@';

    cout << endl
         << endl
         << endl
         << "   ";
    for (int i = 1; i <= 34; i++)

    {

        if (i == 5 || i == 7 || i == 10 || i == 11 || i == 14 || i == 15 ||

            i == 16 || i == 18 || i == 19 || i == 20 || i == 22 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "   ";

    for (int i = 1; i <= 34; i++)

    {

        if (i == 5 || i == 7 || i == 9 || i == 12 || i == 14 || i == 16

            || i == 18 || i == 20 || i == 22 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "   ";

    for (int i = 1; i <= 34; i++)

    {

        if (i == 5 || i == 6 || i == 7 || i == 9 || i == 10 || i == 11 || i == 12 ||

            i == 14 || i == 15 || i == 16 || i == 18 || i == 19 || i == 20 ||

            i == 22 || i == 23 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "   ";

    for (int i = 1; i <= 34; i++)

    {

        if (i == 5 || i == 7 || i == 9 || i == 12 || i == 14 || i == 18 || i == 23)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "   ";

    for (int i = 1; i <= 34; i++)

    {

        if (i == 5 || i == 7 || i == 9 || i == 12 || i == 14 || i == 18 || i == 23)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << endl
         << endl;

    for (int i = 1; i <= 34; i++)

    {

        if (i == 2 || i == 3 || i == 4 || i == 6 || i == 7 || i == 8 || i == 10 ||

            i == 11 || i == 14 || i == 15 || i == 16 || i == 18 || i == 20 ||

            i == 22 || i == 23 || i == 27 || i == 28 || i == 31 || i == 33)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl;

    for (int i = 1; i <= 34; i++)

    {

        if (i == 2 || i == 4 || i == 7 || i == 10 || i == 12 ||

            i == 15 || i == 18 || i == 20 || i == 22 || i == 24 ||

            i == 26 || i == 29 || i == 31 || i == 33)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl;

    for (int i = 1; i <= 34; i++)

    {

        if (i == 2 || i == 3 || i == 4 || i == 7 || i == 10 || i == 11 ||

            i == 15 || i == 18 || i == 19 || i == 20 || i == 22 ||

            i == 24 || i == 26 || i == 27 || i == 28 || i == 29 || i == 31

            || i == 32 || i == 33)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl;

    for (int i = 1; i <= 34; i++)

    {

        if (i == 2 || i == 4 || i == 7 || i == 10 || i == 12 ||

            i == 15 || i == 18 || i == 20 || i == 22 ||

            i == 24 || i == 26 || i == 29 || i == 32)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl;

    for (int i = 1; i <= 34; i++)

    {

        if (i == 2 || i == 3 || i == 4 || i == 6 || i == 7 || i == 8 ||

            i == 10 || i == 12 || i == 15 || i == 18 || i == 20 ||

            i == 23 || i == 22 || i == 26 || i == 29 || i == 32)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << endl
         << endl;

    // keshav row by row

    cout << "\t ";
    for (int i = 1; i <= 24; i++)
    {

        if (i == 1 || i == 3 || i == 5 || i == 6 || i == 7 || i == 9 || i == 10 ||
            i == 11 || i == 13 || i == 15 || i == 18 || i == 19 || i == 22 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "\t ";

    for (int i = 1; i <= 34; i++)
    {

        if (i == 1 || i == 2 || i == 5 || i == 9 || i == 13 || i == 15 || i == 17 ||
            i == 20 || i == 22 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "\t ";

    for (int i = 1; i <= 34; i++)
    {

        if (i == 1 || i == 5 || i == 6 || i == 7 || i == 9 || i == 10 || i == 11 || i == 13 || i == 14 || i == 15 ||
            i == 17 || i == 18 || i == 19 || i == 20 || i == 22 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "\t ";

    for (int i = 1; i <= 34; i++)
    {

        if (i == 1 || i == 2 || i == 5 || i == 11 || i == 13 || i == 15 || i == 17 || i == 20 ||
            i == 22 || i == 24)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << "\t ";

    for (int i = 1; i <= 34; i++)
    {

        if (i == 1 || i == 3 || i == 5 || i == 6 || i == 7 || i == 9 || i == 10 || i == 11 ||
            i == 13 || i == 15 || i == 17 || i == 20 || i == 23)

            cout << ch << " ";

        else
            cout << "  ";
    }

    cout << endl
         << endl
         << endl;

    return 0;
}
