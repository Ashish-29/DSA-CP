...
// hash(s[i....j]) = s[i] + s[i+1]*p + s[i+2]*p^2 +....+ s[j]*p^(j-i) mod M
// hash(s[i....j]) * p^i = s[i]*p^i + s[i+1]*p^(i+1) + s[i+2]*p^(i+2) +....+ s[j]*p^(j) mod M
// hash(s[i....j]) * p^i = ( hash(s[0....j]) - hash(s[0....i-1]) + M) % M  = x (Let)
// hash(s[i....j]) = x * mod_inv(p^i);
...
int main(){
    ...
    f(i,0,s.size()){
        f(j,i,s.size()){
            cout<<i<<" "<<j<<" "<<substring_hash(i,j)<<endl;
        }
    }
